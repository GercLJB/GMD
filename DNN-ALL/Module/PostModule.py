...
def INDEX_MATRIX(Index_df, Matrix, Bool, write_file):
    ...
    if Bool == 'AI': Model = Index_df[Bool].tolist()
    else: Model = Index_df[Bool].tolist()
...
def T_INDEX_ASS(IN_PATH, OUT_PATH, RUN_TIME, MATTER, AREA, TIME_LIST):
    f = open(OUT_PATH + "TIME_INDEX_RESULT.txt", "w")
    f.write("Site" + "\t" + "Time&Day" + "\t" +
            "AI_ACC" + "\t" + "Ratio" + "\t" +
            "AI_POD" + "\t" + "Ratio" + "\t" + "AI_FAR" + "\t" + "Ratio" + "\t" + "AI_F1" + "\t" +
            "N_ACC" + "\t" + "Ratio" + "\t" +
            "N_POD" + "\t" + "Ratio" + "\t" + "N_FAR" + "\t" + "Ratio" + "\t" + "N_F1" + "\n")

    for T in TIME_LIST:
        df = pd.read_csv(IN_PATH + "T{:02d}_Network.txt".format(T), delimiter="\t")
        ...
        CASE4_matrix = [[0 for i in range(5)] for j in range(5)]

        INDEX_MATRIX(Index_df=df, Matrix=AI_matrix, Bool='AI', write_file=f)
        INDEX_MATRIX(Index_df=df, Matrix=CASE4_matrix, Bool='CMAQ', write_file=f)
...
# ----------------------------------------
# Day index agreement calculation
#
...
def T_STT_ASS(IN_PATH, OUT_PATH, RUN_TIME, MATTER, AREA, TIME_LIST):
    ...
    f.write("Site" + "\t" + "Time&Day" + "\t" +
            "AVG_OBS" + "\t" + "AVG_AI" + "\t" + "AVG_N" + "\t" +
            "AI_R" + "\t" + "AI_IOA" + "\t" + "AI_RMSE" + "\t" + "N_R" + "\t" + "N_IOA" + "\t" + "N_RMSE" + "\n")
    ...
    for T in TIME_LIST:
        ...
        AVG_OBS, AVG_AI, AVG_F = (sum(OBS) / len(df)), (sum(AI) / len(df)), (sum(F) / len(df))

        f.write("{}".format(AREA) + "\t" + "T{:02d}".format(T) + "\t" +
                "{:.1f}".format(AVG_OBS) + "\t" + "{:.1f}".format(AVG_AI) + "\t" + "{:.1f}".format(AVG_F) + "\t" +
                "{:.2f}".format(R(OBS, AI)) + "\t" + "{:.2f}".format(IOA(OBS, AVG_OBS, AI)) + "\t" + "{:.1f}".format(RMSE(OBS, AI)) + "\t")
        f.write("{:.2f}".format(R(OBS, F)) + "\t" + "{:.2f}".format(IOA(OBS, AVG_OBS, F)) + "\t" + "{:.1f}".format(RMSE(OBS, F)) + "\n")
